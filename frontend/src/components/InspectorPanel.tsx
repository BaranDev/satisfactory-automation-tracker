...
interface InspectorPanelProps {
  ...
  onSourceItemChange: (machineId: string, item: string | null) => void;
  onSourceRateChange: (machineId: string, rate: number) => void;
...
export default function InspectorPanel({
  ...
  const isItemSource = machine.machineType === "item_source";
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {isItemSource && (
          <div className="space-y-3">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider flex items-center gap-1">
              <Package className="w-3 h-3" /> Source Item
            </label>
            <select
              value={machine.extractionItem ?? ""}
              onChange={e => onSourceItemChange(machine.id, e.target.value || null)}
            ...
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
              Rate ({ITEMS[machine.extractionItem ?? ""]?.isFluid ? "m³" : "items"} / min)
            </label>
            <input
              type="number"
              min={0}
              step={1}
              value={machine.sourceRate ?? 0}
              onChange={e => onSourceRateChange(machine.id, parseFloat(e.target.value) || 0)}
              className="w-full text-xs px-3 py-2 rounded bg-background/80 border border-border/50 text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50 font-mono"
            />

            <p className="text-[10px] text-muted-foreground/70">
              Virtual feeder. Output = rate × overclock. Use to model an external belt
              of items entering this part of the factory (e.g. trains, manual delivery,
              imports from another floor).
            </p>
          </div>
        )}
        ...
        {!isExtractor && !isPowerGen && !isItemSource && compatibleRecipes.length > 0 && (
